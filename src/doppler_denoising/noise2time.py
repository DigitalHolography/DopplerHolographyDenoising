"""Reproducible Noise2Time workflow documented in the repository README.
...
def replacement(record, t, cfg, rng, stage=None):
    ...
    if hasattr(record, "matching_donors"):
        donors = record.matching_donors(t, getattr(record, "donor_bounds", None), exclude)
    else:
        # Compatibility for lightweight external fixtures using the former
        # integer-phase API. Prepared Record objects always take the new path.
        indices = record.donors[int(record.phase[t])]
        if exclude is not None:
            indices = indices[(indices < exclude[0]) | (indices >= exclude[1])]
        donors = [PhaseDonor(int(i), int(i), 0.0, -1) for i in indices if i != t]
    ...
    if cfg.input_mode == "no_patch":
        ...
        ratio = float(record.brightness[t] / donor_brightness) if cfg.brightness_correction else 1.
...
def main(argv=None):
    ...
    p.add_argument("--dataset-measure", help="For one AVI/NPY: measurement folder supplying manual/pseudo masks and report provenance")
    ...
    p.add_argument("--no-progress", action="store_true", help="Disable interactive batch progress bars (used by benchmark subprocesses)")
