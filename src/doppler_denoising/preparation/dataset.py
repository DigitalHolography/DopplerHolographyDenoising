...
LEGACY_KEY = "moment0ff"
SCHEMA = "noise2time.dataset.v1"


def resolve_m0_key(h5):
    """Prefer DopplerView's export; also accept original HoloDoppler files.

    moment0 is deliberately not a fallback: it lacks flat-field correction.
    Both supported layouts store time first. Preserve the selected key in metadata.
    """
    for key in (KEY, LEGACY_KEY):
        if key in h5 and isinstance(h5[key], h5py.Dataset):
            return key
    available=[]
    h5.visititems(lambda name,obj: available.append(name) if isinstance(obj,h5py.Dataset) else None)
    raise ValueError(f"No flat-field M0 dataset in {h5.filename}. Expected '{KEY}' or '{LEGACY_KEY}'. "
...
def prepare_measure(folder, output, args, api):
    ...
    with h5py.File(source, "r") as h5:
        key = resolve_m0_key(h5)
        data = h5[key]
        print(f"  HDF5 dataset: {key}; shape={data.shape}",flush=True)
        if data.ndim != 3 or any(s % 32 for s in data.shape[1:]):
            raise ValueError(f"{key} must be (time,height,width), spatial dimensions divisible by 32; got {data.shape}")
        ...
        try:
            ...
            cycle, fractional_phase, _ = api.cycle_coordinates(len(frames), peaks)
            ...
            table = np.column_stack((np.arange(len(frames)),np.arange(len(frames))/fps,
                                     ...
                                     phase,cycle,fractional_phase,valid,result["artifact_mask"]))
            np.savetxt(output/"brightness.csv",table,delimiter=",",comments="",
                       header="frame,time_seconds,raw_arterial_M0,brightness_smoothed_M0,detection_smoothed_M0,phase_frames,cycle,fractional_phase,valid_target_or_donor,artifact")
            ...
            plot_cycles(output/"brightness.png", arterial*scale, brightness*scale, result, scale, fps, folder.name)
...
def plot_cycles(path, raw, brightness, result, scale, fps, title=None):
    figure = Figure(figsize=(13,5), layout="constrained"); FigureCanvasAgg(figure)
    ax = figure.subplots(); t=np.arange(len(raw))
    peaks=result["peaks"]
    for index,(start,stop) in enumerate(zip(peaks[:-1],peaks[1:])):
        ax.axvspan(start,stop,color=("#d7edf3" if index%2==0 else "#f4ead5"),alpha=.5)
        ax.text((start+stop)/2,.98,f"Cycle {index+1}",transform=ax.get_xaxis_transform(),ha="center",va="top")
    ...
    ax.set(xlabel="Frame number (zero-based, original video)",ylabel="M0 intensity",
           title=f"{title or Path(path).parent.name}\nArterial signal, peaks and complete cycles")
